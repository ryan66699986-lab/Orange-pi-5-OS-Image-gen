...
say "Generating offline H.264/HEVC hardware-decode probes"
mkdir -p "$WORK/probe"
docker run --rm --platform linux/amd64 -v "$WORK/probe:/out" ubuntu:26.04 bash -ceu '
  ...
  test -s /out/h264-hwprobe.mp4; test -s /out/hevc-hwprobe.mp4
'
mkdir -p "$USERPATCHES_DIR/overlay"
printf '%s\n' "$PI_PASS_HASH" > "$WORK/user-password.hash"; unset PI_PASS_HASH
docker run --rm --platform linux/amd64 -v "$OVERLAY:/src:ro" -v "$USERPATCHES_DIR/overlay:/dst" -v "$LOCK:/meta/versions.lock.json:ro" -v "$WORK/required-packages.txt:/meta/required-packages.txt:ro" -v "$WORK/user-password.hash:/meta/user-password.hash:ro" -v "$WORK/probe/h264-hwprobe.mp4:/meta/h264-hwprobe.mp4:ro" -v "$WORK/probe/hevc-hwprobe.mp4:/meta/hevc-hwprobe.mp4:ro" ubuntu:26.04 bash -ceu '
