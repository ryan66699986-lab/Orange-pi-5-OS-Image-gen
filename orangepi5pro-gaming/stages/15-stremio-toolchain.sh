...
docker run --rm --platform linux/arm64 ubuntu:26.04 bash -ceu '
  ...
  apt-get install -y --no-install-recommends pkg-config libgtk-4-dev libadwaita-1-dev libwebkitgtk-6.0-dev rustc cargo >/dev/null
  ...
  printf "GTK=%s Adwaita=%s WebKitGTK=%s rustc=%s\n" "$(pkg-config --modversion gtk4)" "$(pkg-config --modversion libadwaita-1)" "$(pkg-config --modversion webkitgtk-6.0)" "$rustver"
'
