...
docker run --rm -i --privileged --platform linux/amd64 -v "$SOURCE_IMAGE:/image.img:ro" ubuntu:26.04 bash -seu <<'OFFLINE_QA'
  ...
  for pkg in libsdl3-0 libsdl3-ttf0 mesa-vulkan-drivers armbian-firmware f2fs-tools xfsprogs; do grep -qx "Package: $pkg" "$R/var/lib/dpkg/status" || { echo "Final image package database lacks $pkg" >&2; exit 1; }; done
