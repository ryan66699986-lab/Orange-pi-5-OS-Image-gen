...
docker run --rm -i --privileged --platform linux/amd64 -v "$SOURCE_IMAGE:/image.img:ro" ubuntu:26.04 bash -seu <<'OFFLINE_QA'
  ...
  maintenance_required=(/etc/opi/component-policy.tsv /etc/update-manager/release-upgrades /usr/local/bin/opi-direct-gaming-session /usr/local/bin/opi-appliance-health /usr/local/sbin/opi-update /usr/share/wayland-sessions/opi-direct-gaming.desktop /usr/share/applications/opi-system-update.desktop /home/ryan/ROMs/ports/Direct-Gaming-Mode.sh /home/ryan/ROMs/ports/Check-System-Updates.sh)
  for f in "${maintenance_required[@]}"; do [[ -e "$R$f" ]] || { echo "V3.27 appliance policy missing from final image: $f" >&2; exit 1; }; done
  ...
  grep -Fq 'exec /usr/local/bin/opi-direct-gaming-session' "$R/usr/local/bin/opi-gaming-session" || { echo "Final image lacks direct Labwc fallback" >&2; exit 1; }
  grep -Fq 'OPI_DIRECT_GAMING=1' "$R/usr/local/bin/opi-direct-gaming-session" || { echo "Final direct gaming session marker missing" >&2; exit 1; }
  for tier in core required experimental; do grep -q "^${tier}[[:space:]]" "$R/etc/opi/component-policy.tsv" || { echo "Final component policy lacks ${tier} tier" >&2; exit 1; }; done
  grep -qx 'Prompt=never' "$R/etc/update-manager/release-upgrades" || { echo "Final image permits an unreviewed distribution upgrade" >&2; exit 1; }
  grep -Fq 'apt-get -s upgrade' "$R/usr/local/sbin/opi-update" || { echo "Final updater lacks APT simulation" >&2; exit 1; }
  grep -Fq 'apt-get upgrade' "$R/usr/local/sbin/opi-update" || { echo "Final updater lacks same-release package upgrade" >&2; exit 1; }
  ! grep -Eq 'dist-upgrade|full-upgrade|do-release-upgrade' "$R/usr/local/sbin/opi-update" || { echo "Final updater contains a forbidden release/removal-capable path" >&2; exit 1; }
